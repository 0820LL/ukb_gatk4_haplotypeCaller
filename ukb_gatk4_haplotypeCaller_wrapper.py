...
def steward(config_file_path:str, ukb_gatk4_haplotypeCaller_path:str, send_message_script:str) -> None:
    ...
    ukb_gatk4_haplotypeCaller_command = 'nextflow run -offline -profile singularity -bg -params-file {} {} >> run_log.txt'.format(params_file_path, ukb_gatk4_haplotypeCaller_path)
    return_value = os.system(ukb_gatk4_haplotypeCaller_command)
    with open('ukb_gatk4_haplotypeCaller_command.txt', 'w') as command_f:
        command_f.write(ukb_gatk4_haplotypeCaller_command + '\n')
    ...
    feedback_dict = {
        ...
        'pipelineName'    : 'ukb_gatk4_haplotypeCaller',
        'analysisStatus'  : '',
    ...
    while True:
        with open(execution_trace_file, encoding = 'UTF-8') as trace_f:
            for line in trace_f:
                if 'GATK4_HAPLOTYPECALLER' in line:
                    if 'COMPLETED' in line:
                        feedback_dict['endDate'] = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())
                        feedback_dict['error'] = 0
                        for file in os.listdir('{}/results'.format(analysis_path)):
                            if file.endswith('.vcf.gz'):
                                vcf_file = '{}/results/{}'.format(analysis_path, file)
                            if file.endswith('vcf.gz.tbi'):
                                vcf_file_tbi = '{}/results/{}'.format(analysis_path, file)
                        report_download_dict = {
                            ...
                            'text' : [
                                {
                                    'sort'   : 1,
                                    'title'  : 'GATK4 HaplotypeCaller检测SNP和Indels结果1',
                                    'content': 'GATK4 HaplotypeCaller检测SNP和Indels结果1：#&{}'.format(vcf_file),
                                    'postDes': '',
                                    'memo'   : '',
                                    'preDes' : ''
                                },
                                {
                                    'sort'   : 2,
                                    'title'  : 'GATK4 HaplotypeCaller检测SNP和Indels结果2',
                                    'content': 'GATK4 HaplotypeCaller检测SNP和Indels结果2：#&{}'.format(vcf_file_tbi),
                                    'postDes': '',
                                    'memo'   : '',
                                    'preDes' : ''
                                }
...
def main() -> None:
    ...
    parser.add_argument('--ukb_gatk4_haplotypeCaller_path', required=True, help='the full path of the ukb_gatk4_haplotypeCaller')
    ...
    ukb_gatk4_haplotypeCaller_path = args.ukb_gatk4_haplotypeCaller_path
